...
func (o *TravelBehavior) Run(cli *service.SvcImpl) {
	...
	fmt.Println(QueryAllTravelInfo)
	...
	// 3. Query Again
	QueryAllTravelInfoAgain, err2 := travelSvc.QueryAll()
	if err2 != nil {
		fmt.Println("error2 occurs: %v", err2)
	}
	fmt.Println(QueryAllTravelInfoAgain)
	time.Sleep(2 * time.Second)
	...
	if err3 != nil {
		fmt.Println("error3 occurs: %v", err3)
	}
	time.Sleep(2 * time.Second)
